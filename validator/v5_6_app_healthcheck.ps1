...
$mywlst_hc2 = "D:\ORA\MWFR\12cappr2\oracle_common\common\bin\wlst.cmd $sc_path\status.py"
...
$transcript = "transcript_v56_" + $hostname + "_" + $mydate + ".txt" 
$check_admin_server = "D:\ORA\user_projects\domains\OperaDomain\servers\AdminServer\data\nodemanager\AdminServer.state"
$report= "report_v56_"+ $hostname + "_" + $mydate + ".txt"
$myactions = "actions_v56_"+ $hostname + "_" + $mydate + ".txt"
$elk_log = "cv_v56_"+ $hostname + ".txt" 
...
function check_opera_version {  
  ...
  if($major_version -eq "5.0.06") {
   Write-Host "Opera : $major_version and script works" -ForegroundColor Yellow
  }else {
    Write-Host "Not an Opera v5_6 and script is intent to work for version : $major_version" -ForegroundColor Red
    break
   }
   write-host ""
} 
...
function update_py {
  ...
  #(Get-Content $sc_path\status.py) -replace 'defaultpassword', "$pass" | Set-Content $sc_path\status.py
  #if ($?) { 
  ...
  (Get-Content $sc_path\status.py) -replace 'myhostname', "$myhost" | Set-Content $sc_path\status.py
  if ($?) { 
...
function prepare_and_execute {
  ...
  #Remove-Item $sc_path\output\report_v5*.txt | Out-Null
  Remove-Item $sc_path\logs\output_*.ini | Out-Null
  Remove-Item $sc_path\logs\*_status.txt | Out-Null 
  #Remove-Item $sc_path\output\actions_v5*.txt | Out-Null
...
function Report_server_env {
  ...
  [string[]]$sites = ( 
    "https://$myhost/reports/rwservlet/showenv?server=rep" + $hostname + "OPERA"
 )
 [Net.ServicePointManager]::SecurityProtocol = [Net.SecurityProtocolType]::Tls12
 #$securepassword = ConvertTo-SecureString "$pass" -AsPlainText -Force
 #$credentials = New-Object System.Management.Automation.PSCredential("weblogic", $securepassword)
...
function Report_server_info {
  ...
  [string[]]$sites = ( 
    "https://$myhost/reports/rwservlet/getserverinfo?server=rep" + $hostname + "OPERA"
 )
 [Net.ServicePointManager]::SecurityProtocol = [Net.SecurityProtocolType]::Tls12
 #$securepassword = ConvertTo-SecureString "$pass" -AsPlainText -Force
 #$credentials = New-Object System.Management.Automation.PSCredential("weblogic", $securepassword)
...
function Report_server_jobs {
  ...
  [string[]]$sites = ( 
    "https://$myhost/reports/rwservlet/showjobs?server=rep" + $hostname + "OPERA"
 )
 [Net.ServicePointManager]::SecurityProtocol = [Net.SecurityProtocolType]::Tls12
 #$securepassword = ConvertTo-SecureString "$pass" -AsPlainText -Force
 #$credentials = New-Object System.Management.Automation.PSCredential("weblogic", $securepassword)
...
function ifc8ws_status {
  ...
  [string[]]$sites = ( 
    "http://" + $myhost + ":6003/Operajserv/Ifc8ws/Ifc8ws"
 )
 [Net.ServicePointManager]::SecurityProtocol = [Net.SecurityProtocolType]::Tls12
 #$securepassword = ConvertTo-SecureString "$pass" -AsPlainText -Force
 #$credentials = New-Object System.Management.Automation.PSCredential("weblogic", $securepassword)
...
function revert_py {
  ...
  (Get-Content $sc_path\status.py) -replace "$myhost", 'myhostname' | Set-Content $sc_path\status.py
  if ($?) { 
  ...
  #(Get-Content $sc_path\status.py) -replace "$pass", 'defaultpassword' | Set-Content $sc_path\status.py
  #if ($?) { 
